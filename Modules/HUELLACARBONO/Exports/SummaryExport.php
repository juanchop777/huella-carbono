...
class SummaryExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle
{
    ...
    {
        $byMonth = $this->consumptions->groupBy(function ($item) {
            return $item->consumption_date->format('Y-m');
        })->map(function ($items, $period) {
            return (object) [
                'period' => $period,
                'period_label' => $items->first()->consumption_date->format('m/Y'),
                'count' => $items->count(),
    ...
    {
        return ['Período (Mes)', 'Registros', 'Total CO₂ (kg)'];
    }
